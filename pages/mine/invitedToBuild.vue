<template>
    <view class="page">
        <view class="content">
			<image v-if="user.headImg" mode="aspectFill" class="avatar" :src="user.headImg"></image>
			...
			<view class="condition">
				<view :class="[user.isCheck==1||user.isBuildMine==1?'':'titleType','isCheck']">
					<text>通过身份认证</text>
					<image v-if="user.isCheck==1||user.isBuildMine==1" src="../../static/mine/xz_3x.png"></image>
				</view>
				<view :class="[user.isConform==1||user.isBuildMine==1?'':'titleType','isConform']">
					<text>持有{{user.needAmount}}BTA及以上</text>
					<image v-if="user.isConform==1||user.isBuildMine==1" src="../../static/mine/xz_3x.png"></image>
				</view>
			</view>
			<view class="hint" v-if="!userStatus">
				该用户必须满足以上所有条件
			</view>
            <view  :class="[userStatus?'':'opacitySize',user.isBuildMine==1?'opacitySize':'','btn-primary','button']" @click="is_showModal">{{user.isBuildMine==1?'邀请已发送':'邀请该用户建矿'}}</view>
        </view>
		<view class="bottom-title">您的邀请需要用户在任务列表中同意后方可生效</view>
...
<script>
    ...
    export default {
        ...
        methods: {
			//判断bat是否够
			getIsConform(user){
				this.$api.meetBuildMine(user.userId).then(result=>{
					user.isConform=result.isConform;
					user.needAmount=result.needAmount;
					this.user = user;
				}).catch(error => {
						this.status='network';
            ...
            onConfirm () {
                ...
                uni.showLoading();
                this.$api.setBuildMine(this.user.userId).then(result => {
                    uni.hideLoading();
					this.showToast('邀请已发送');
					this.user.isBuildMine=1;
...
<style scoped lang="less">
    ...
    .button {
        line-height: 98upx;
        margin:62upx 32upx 32upx 32upx ;
        font-size:38upx;
