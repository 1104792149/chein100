    ...
    <view class="page">
        ...
        <view v-if="list.length>0" class="user-list">
            <view class="user" v-for="(user,index) in list" :key='index'>
                ...
                <view class="button" @click="confirmSetting(user)">设置</view>
            </view>
        </view>
...
<script>
    ...
    export default {
        ...
        methods: {
			initTap(){
				 this.$api.queryInviteUsers(this.currentPage, this.searchKey).then(result => {
					...
					if(this.list.length>0){
						...
						this.customEmptyText='暂时没有符合辅助节点条件的用户';
						this.status='empty';
					}
					//上拉加载
            ...
            confirmSetting (user) {
                this.navigateTo('./SetAssistNodeConfirm', user)
            },
            search () {
