    ...
    export default {
        ...
        data () {
            return {
				...
				status:'',
				customEmptyText:'',
				loadingType2:0,//1开启，0停止，2没有更多数据
				typeNumber:1,//是否已建矿：1未建矿，2已建矿
				BuildNumber:{
					limitNum:20,
					num:0
				}
        ...
        methods: {
			initTap(){
				if(this.searchKey){
					this.$api.queryBuildMineUsers(this.currentPage, this.searchKey).then(result => {
					   this.disposeTap(result)
					}).catch(error => {
						this.status='network';
					    this.loadMoreErrorHandle(error);
					})
				}else{
					this.$api.typeBuildMineUsers(this.currentPage, this.typeNumber).then(result => {
					   this.disposeTap(result)
					}).catch(error => {
						this.status='network';
					    this.loadMoreErrorHandle(error);
			...
			//跳转详情
            confirmSetting (user) {
				if(user.isBuildMine==2)return;
                this.navigateTo('./invitedToBuild', user) 
            },
			//搜索
            search () {
				if(this.searchKey){
					this.searchStatus=false
					this.pageScrollToTap();
				}else{
					this.isTypeTap(1);
				}
            },
			...
			//获取已有多少人建矿 
			countBuildMineNum(){
				this.$api.countBuildMineNum().then(result => {
				  this.BuildNumber=result;
				}).catch(error => {
				    this.loadMoreErrorHandle(error);
				})
			}
...
<style scoped lang="less">
	...
	.invitedType{
		display: flex;flex-direction: row;align-items: center;justify-content: center;color: #999;font-size: 30upx;margin-top: 52upx;
		}
	.invitedTypeText{
		color: #222222;
	}	
